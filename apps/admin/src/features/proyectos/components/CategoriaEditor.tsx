...
export function CategoriaEditor({ categoriaId, onBack }: CategoriaEditorProps) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Actualizar imagen de portada en categories
      const { error: categoryError } = await supabase
        .from('categories')
        .update({ cover_image_url: form.imagen_portada })
        .eq('id', categoriaId);
